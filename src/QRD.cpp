...
// Destructor 
QRD::~QRD(){Free(_qr); Free(_work); Free(_tau); Free(_pivot); }
...
QRD::QRD(const Matrix& A, const double tol=1E-12) 
{
  ...
  _qr=C2F(A);
  _pivot=(int*)Calloc(_p,int); 
  _tau=(double*)Calloc(_p,double); 
  _work=(double*)Calloc(_p*2,double);
  for(int i=0;i<_p;i++) {_pivot[i]=i+1; _tau[i]=0; _work[i]=0; _work[i+_p]=0; }
...
ReturnMatrix QRD::Q() const
{
  ...
  y=(double *)Calloc(n*n,double); q=(double *)Calloc(n*n,double); 
  for(i=0;i<n;i++)
    for(j=0;j<n;j++){ y[ct++] = (i==j) ? 1 : 0; q[ct-1]=y[ct-1]; }

  // Compute full orthogonal factor Q(n,n) 
  F77_CALL(dqrqy)(_qr, &n, &rank, _tau, y, &n, q); 
  
  // Convert FORTRAN to C and return
  Matrix Qout=F2C(q,n,n); Free(y); Free(q); 
  Qout.Release(); return Qout.ForReturn(); 
}
...
ReturnMatrix QRD::R() const
{
  ...
  Matrix R(minnp,_p); R=0.0; double *r=R.Store();
  for(i=0;i<minnp;i++) 
    for(j=i;j<_p;j++) r[i*_p+j] = _qr[j*_n+i]; 
  R.Release(); return R.ForReturn(); 
}
...
ReturnMatrix QRD::Solve(const Matrix &Y) const
{
  ...
  if(_n<1 || _rank!=_p || Y.Nrows()!=_n)
    { Matrix t(1,1); t=-1; t.Release(); return t.ForReturn(); }

  // Assign input values and allocate memory
  int i, n=_n, k=_rank, ny=Y.Ncols(), info[1]; info[0]=1;
  double *y=C2F(Y), *coef=(double*)Calloc(k*ny,double); 
  for(i=0;i<k*ny;i++) coef[i]=0.0; 
  ...
  Matrix out=F2C(coef,k,ny); Free(coef); Free(y); 
