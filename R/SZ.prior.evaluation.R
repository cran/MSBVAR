...
##                                   lambda4, lambda5, mu5, mu6, z=NULL,
##                                   nu=ncol(Y)+1, qm, prior=0, nstep,
##                                   y.future)
## {
##     combos <- length(lambda0)*length(lambda1)*length(lambda3)*length(lambda4)*length(lambda5)*length(mu5)*length(mu6)
##     results <- matrix(0, combos ,11)
##     h <- 0
...
##             { for(m in 1:length(lambda5))
##               { for(n in 1:length(mu5))
##                 { for(o in 1:length(mu6))
##                   {
##                       fit <- szbvar(Y, p, z,
##                                     lambda0[i],
##                                     lambda1[j],
...
##                       forecast <- forecast.VAR(fit, nstep)
##                       eval.forecasts <- cf.forecasts(forecast[(nrow(forecast)-nstep+1):nrow(forecast),], y.future)

##                       # Now gather up all the parameters and results
##                       tmp <- c(lambda0[i],lambda1[j],lambda3[k],lambda4[l],
##                                lambda5[m], mu5[n], mu6[o], eval.forecasts[1],
